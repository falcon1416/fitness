<template>
	<div class="page has-navbar" v-nav="{title: '类型', showBackButton: false,showMenuButton:true,menuButtonText:menuButtonText,onMenuButtonClick:onShowPopup}">
		<div class="page-content">
			<!--数据列表-->
			<list class="list-ios">
				<item v-for="item in list" :key="item.id" class="item item-icon-right" @click.native="is_del==0?onListClick(item):''">
					{{item.name}}

					<i v-if="is_del==0" class="icon ion-ios-arrow-right" style="color: #DDD;"></i>
					<i v-else class="icon ion-close-circled" style="color:crimson;" @click="onDel(item)"></i>
				</item>
			</list>
		</div>
	</div>
...
<script>
	...
	export default {
		data() {
			return {
				menuButtonText:
					'<a class="button button-icon icon  ion-ios-more"></a>',
				list: [],
				parent_id: 0,
